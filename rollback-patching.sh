...
# ------------------------------------------------------------------
  ...
  while :
  do
    status=$(aws cloudformation describe-change-set --stack-name ${s} install-nodegroup-updates | jq -r '.ExecutionStatus')
...
now=$(date +'%Y-%m-%d')
mv ${account_alias}-files ${account_alias}-files-${now}
mkdir ${account_alias}-files
